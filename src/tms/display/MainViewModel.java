...
public class MainViewModel {
    ...
    public void save(String filename) {
        try (BufferedWriter writer =
            ...
            error.setValue("Error occurred when opening or writing to the " +
                    "file.");
        }
    }
    ...
    public void togglePaused() {
        ...
        pausedText.setValue("System Paused: " +
                pausedProperty.getValue().toString());

        // If paused, set text to "Unpause", and if not paused, set text to
        // "Pause"
        String pauseOrUnpause = (pausedProperty.get()) ? "Unpause" :
                "Pause";
        pausedButtonText.setValue(pauseOrUnpause);
    }
     ...
     */
    public void tick() {
        // Only take action when not paused
        if (!pausedProperty.get()) {
            TimedItemManager.getTimedItemManager().oneSecond();
            seconds.setValue(seconds.get() + 1);
            timeElapsed.setValue("Seconds Elapsed: " +
                    seconds.getValue().toString());
    ...
    void accept(String input) {
        // Both upper and lowercase keys should take same action, hence empty
        // cases (and lack of break statements)
        switch (input) {
            case "P":
            case "p":
                this.togglePaused();
