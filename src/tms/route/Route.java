...
public class Route {
    ...
    /** Congestion calculator that calculates the average congestion value
    ...
    /**
     * (Helper Method)
     * Returns true if this route has a traffic light; false otherwise.
     *
     * @return whether a traffic light is present on this route
     */
     ...
     *
     * <p>By default, an AveragingCongestionCalculator should be used to find
     * the route's congestion level. See <code>CongestionCalculator
     * .calculateCongestion()</code>.</p>
     *
     * @return the congestion level on this route as returned by the calculator
     */
    public int getCongestion() {
        return this.averagingCongestionCalculator.calculateCongestion();
    }
     ...
     *
     * <p>For two routes to be equal, they must: </p>
     *
     * <ul>
     *     <li>have the same identifier string</li>
     *     <li>have the same default speed</li>
     *     <li>either both have a traffic light signal or both have no
     *     traffic light signal</li>
     *     <li>have the same traffic light signal status (GREEN, RED, etc.)</li>
     ...
     *     sensor's <code>equals()</code> method, directly or indirectly)</li>
     * </ul>
    ...
    public boolean equals(Object obj) {
        if (obj != null && obj.getClass() == this.getClass()) {
            ...
            if (route.id.equals(this.id) &&
                    route.defaultSpeed == this.defaultSpeed &&
                    route.hasTrafficLight() == this.hasTrafficLight() &&
                    this.hasSpeedSign() == route.hasSpeedSign() &&
                    this.getSpeed() == route.getSpeed()) {
                // Ensure both routes have traffic lights to check matching
                // signals
                if (this.hasTrafficLight()) {
                    if (this.getTrafficLight().getSignal() ==
                            route.getTrafficLight().getSignal()) {
                        return (this.sensors.containsAll(route.sensors) &&
                                route.sensors.containsAll(this.sensors));
                    }
                } else {
                    return (this.sensors.containsAll(route.sensors) &&
                            route.sensors.containsAll(this.sensors));
     ...
     *
     * <p>Two routes that are equal must have the same hash code.</p>
     * @return hash code of the route
     */
    public int hashCode() {
        int hashCodeReturn = Objects.hash(id, speedSign, trafficLight,
                defaultSpeed);

        // hash sensors individually and then sum to ensure order of sensors
        // list does not affect uniqueness of hashCode
        for (Sensor sensor : this.getSensors()) {
            hashCodeReturn += Objects.hash(sensor);
        }
